...
const tokenExtractor = (req, res, next) => {
	...
	if (!authorization || !authorization.toLowerCase().includes("bearer "))
		req.token = undefined
	else req.token = req.get("Authorization").substring("Bearer ".length)
...
const userExtractor = async (req, res, next) => {
	...
	req.credentials = jwt.verify(req.token, process.env.SECRET, {
		algorithm: "HS256",
	})
...
app.use((err, req, res, next) => {
	...
	if (err.name === "CastError") res.status(400).json({ error: err.message })
