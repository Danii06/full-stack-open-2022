...
userRouter.post("/",async (request, response) => {
	...
	if(newUserJSON.password.toString().length<3){
		response.status(400).json({ error: "Password too short (minimum length is 3 characters)" })
		return
	}
	...
	if(matchingUsers.length>0){
		response.status(422).json({ error: "A user with that username already exists" })
		return
	}
